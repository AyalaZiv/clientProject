...
class SessionWithClient(threading.Thread):
 ...
 #       self.operFnPtrDict = { 1 : self.oper1Fun, 2 : self.oper1Fun }
    ...
    def run(self):
        try:               
           ...
           # self.pythonServer.gui.guiSock.send("Hello " +  self.addr[0] + "#")   # to GUI
            ...
            connections_client_list = self.get_connections_client_list()
            print connections_client_list
            for con in connections_client_list:
                connection_maker = con.get_connection_maker()
                if connection_maker == "yes":
                    self.connection_makers.append(con)
                else:
                    self.got_connection.append(con)
    ...
    def get_connections_client_list(self):
        connection_list = self.rec_long()
        pickled_connection_list = pickle.loads(connection_list)
        return pickled_connection_list
